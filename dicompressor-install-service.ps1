<#
.SYNOPSIS
    Interactive NSSM installer for the generic DicomPressor Windows service.

.DESCRIPTION
    Prompts for the important paths and watch interval, then creates an auto-start
    hidden Windows service that runs dicompressor.py in merge watch mode.
...
Write-Host "DicomPressor Windows Service Installer" -ForegroundColor Cyan
...
Write-Step "Service"
$serviceName = Prompt-Default -Prompt "Service name" -Default "DicomPressorGeneric"
$displayName = Prompt-Default -Prompt "Display name" -Default "DicomPressor Generic"
...
$description = "DicomPressor generic merge watch service. Source=$sourceDir Output=$outputDir Interval=${intervalSeconds}s"
